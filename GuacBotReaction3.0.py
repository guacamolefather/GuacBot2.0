    ...
    for trigger in triggersList:
        if not isinstance(trigger, str):
            for subtrigger in trigger:
                if (" " + subtrigger) in message.content.lower() or (message.content.lower().startswith(subtrigger) and (subtrigger + " ") in message.content.lower()) or subtrigger == message.content.lower():
                    reaction = charLimit(triggersList.index(trigger), responsesList)
                    for i in reaction:
                        await message.channel.send(i)
        else:
            if (" " + trigger) in message.content.lower() or (message.content.lower().startswith(trigger) and (trigger + " ") in message.content.lower()) or trigger == message.content.lower():
                reaction = charLimit(triggersList.index(trigger), responsesList)
                for i in reaction:
                    await message.channel.send(i)
